...
MIN_PRICE = 0              # 가격 제한 없음 (필요시 수정)
DROP_THRESHOLD = -1.5      # -1.5% 이하로 하락한 종목 (더 많이 떨어진 것들)
REPORT_FILE = "kospi_drop_stocks.html"

def collect_drop_stocks():
    results = []
    try:
        with sync_playwright() as p:
            print(f"📊 코스피 하락주(등락률 {DROP_THRESHOLD}% 이하) 분석 중...")
            ...
            for row in rows:
                try:
                    ...
                    # --- [하락 필터링 로직] ---
                    # 설정한 하락폭(예: -3.0)보다 더 많이 떨어진(더 작은) 경우만 수집
...
def generate_html(data):
    ...
    <head>
        <meta charset="UTF-8">
        <title>코스피 하락 종목 리포트</title>
        <style>
            body {{ font-family: 'Malgun Gothic', sans-serif; background-color: #f0f4f8; padding: 30px; }}
            ...
            .stock-card:hover {{ background-color: #f8fbff; border-color: #adc6ff; }}
    ...
    <body>
        <div class="container">
            <h1>📉 하락 종목 리포트</h1>
            <div class="subtitle">등락률 {DROP_THRESHOLD}% 이하 (과매도 확인용)</div>
    """
    if not data:
