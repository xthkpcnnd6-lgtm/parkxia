...
UP_THRESHOLD = 3.0         # +3.0% 이상 상승한 종목
REPORT_FILE = "kosdaq_up_stocks.html"

def collect_kosdaq_up_stocks():
    results = []
    try:
        with sync_playwright() as p:
            print(f"📊 코스닥 상승주(등락률 {UP_THRESHOLD}% 이상) 분석 중...")
            browser = p.chromium.launch(headless=True)
            context = browser.new_context(user_agent="Mozilla/5.0")
            page = context.new_page()
            
            # 네이버 증권 코스닥 시가총액 페이지 (sosok=1 이 코스닥입니다)
...
def generate_html(data):
    ...
    <head>
        <meta charset="UTF-8">
        <title>코스닥 상승 종목 리포트</title>
        <style>
            body {{ font-family: 'Malgun Gothic', sans-serif; background-color: #fff9f9; padding: 30px; }}
            ...
            .stock-card:hover {{ background-color: #fffefe; border-color: #ffcfcf; }}
            .name {{ font-size: 1.15rem; font-weight: bold; color: #222; }}
            .price {{ font-size: 1.3rem; font-weight: 800; color: #d93025; }}
            .up-badge {{ display: inline-block; padding: 4px 10px; background: #d93025; color: white; border-radius: 20px; font-size: 0.85rem; font-weight: bold; margin-top: 5px; }}
    ...
    <body>
        <div class="container">
            <h1>🚀 KOSDAQ 상승 종목</h1>
            <div class="subtitle">등락률 +{UP_THRESHOLD}% 이상 종목 포착</div>
    """
    if not data:
        html_content += "<p style='text-align:center; padding: 50px; color:#888;'>현재 조건에 맞게 상승한 종목이 없습니다. ☁️</p>"
    else:
        for item in data:
            html_content += f"""
                <div class="stock-card">
                    <div>
                        ...
                        <a href="{item['link']}" class="link-btn" target="_blank">상세정보 보기</a>
                    </div>
                    <div style="text-align: right;">
                        <div class="price">{item['price']}</div>
                        <div class="up-badge">{item['change']}</div>
                    </div>
                </div>
            """
